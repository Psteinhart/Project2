node('master'){
    ...
    stage('Deploy'){
        try{
			...
			bat 'MSDeploy.exe -verb:sync -source:contentPath="C:\\Program Files (x86)\\Jenkins\\workspace\\Jenkinops\\Chatbot\\Chatbot.angularClient\\dist" -dest:iisApp="Default Web Site/jenkinsops",wmsvc=ec2-54-152-165-243.compute-1.amazonaws.com,username=Administrator,password=Pizza1234 -allowUntrusted -enableRule:AppOffline"'
    } catch(error) {
      //slackSend message: color:'danger'
    }
  }
